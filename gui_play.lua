...
local args = {...}
local playlist = {}
local directory = args[1] or shell.dir()

-- Resolve playlist
if fs.isDir(directory) then
    for _, file in ipairs(fs.list(directory)) do
        if file:match("%.dfpwm$") then
            table.insert(playlist, fs.combine(directory, file))
        end
    end
else
    if directory:match("%.dfpwm$") and fs.exists(directory) then
        table.insert(playlist, directory)
    end
end

if #playlist == 0 then
    print("No .dfpwm files found in " .. directory)
    return
end
...
local function drawUI()
    ...
    local songName = fs.getName(playlist[currentSongIdx] or "")
...
local function audioLoop()
    while not exitProgram do
        if isPlaying then
            ...
            local file = fs.open(currentFile, "rb")
